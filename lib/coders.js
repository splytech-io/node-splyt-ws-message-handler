...
function decode(encoding, data, options = {}) {
  if (!encoding) {
  ...
  return co(function *() {
    for (let i = encoding.length - 1; i >= 0; i--) {
      ...
      data = yield coders[encodingParams.coder].decode(data, _.extend({}, encodingParams.options, options));
    }
...
function encode(data, options) {
  ...
  return co(function *() {
    ...
    if (options.pkcrypt) {
      str = yield coders.pkcrypt.encode(str, options.pkcrypt);
